                ...
                <table class="table ms-5 ps-5 mb-5 mt-5">
                    <thead>
                      <tr>
                        ...
                        <th>Email</th>
                        <th>Visibilité</th>
                        <!-- Ajout de la colonne Action (modifier/supprimer) -->
                        <th>Action</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                          ...
                          <td>{{ $item['email'] }}</td>
                          <td>{{ $item['visibilite'] ?? '' }}</td>
                          <!-- Nouvelle cellule Action: modifier/supprimer -->
                          <td>
                            <a href="{{ route('commandes.edit', $item['id']) }}" class="btn btn-warning btn-sm">
                              Modifier<i class="fas fa-edit ms-1"></i>
                            </a>
                            <form action="{{ route('commandes.destroy', $item['id']) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette commande ?');">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-danger btn-sm">
                                Supprimer<i class="fas fa-trash ms-1"></i>
                              </button>
                            </form>
                          </td>
                          <td>
                            <button class="btn btn-toggle-products" data-id="{{ $item['id'] }}">
                              voir<i class="fas fa-eye ms-2" style="color:green"></i>
                            </button>
                          </td>
                      </tr>
                      <tr class="product-row" data-id="{{ $item['id'] }}">
                        <!-- Ajustement du colspan pour inclure la nouvelle colonne (8 colonnes maintenant) -->
                        <td colspan="8">
                          <table class="table table-sm " style="background-color:azure">
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td>{{ $val }}</td>
                                <td>
                                  <button class="btn btn-toggle-products5" data-id="{{ $item['id'] }}">
                                    Traiter<i class="fas fa-check ms-1" style="color:green"></i>
                                  </button>
                                </td>
                              </tr>
                            </tbody>
